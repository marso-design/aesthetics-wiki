---
name: "Renaissance Revival"
slug: "renaissance-revival"
decade_of_origin: "1990s"
key_motifs: ["Corsets", "Elizabethan ruffs", "bodices", "long dresses", "fairies"]
key_colours: ["Gold", "Brown", "sepia", "black"]
palette: ["#161515", "#E5C884", "#D57A27", "#D94F19", "#753121", "#E2AC5C"]
related_aesthetics: ["Baroque", "Ethereal", "Fairycore", "Frasurbane", "FantasY2K", "Hellenic", "Italian Renaissance", "Mid-Century Medieval", "Sepia Blur", "Tudor", "Vintage", "Whimsigoth"]
related_media: ["Ever After (1998)", "Romeo + Juliet", "Elizabeth (1998)"]
source_url: "https://aesthetics.fandom.com/wiki/Renaissance_Revival"
license: "CC-BY-SA-4.0"
retrieved: "2026-07-22"
image_count: 10
---

# Renaissance Revival

Needs Work 🛠️

This page requires improvement. Please help us by [expanding it](https://aesthetics.fandom.com/wiki/Renaissance_Revival?action=edit). If you are unsure how to help, check the article guide [Format and Content](https://aesthetics.fandom.com/wiki/Format_and_Content "Format and Content").

**Renaissance Revival** refers to a trend originating from the 1990s that reinterprets elements from fashion and art of the Renaissance through a cotemporary lens.

## Visuals

The aesthetic is defined by imagery reminiscent of the Renaissance era, drawing inspiration from fairytales, Greco-Roman paintings and art styles from the Middle Ages and Elizabethan period. Visuals often use warm muted colors, sepia filters/textures and renaissance script fonts to evoke a sense of antique quality.

## Fashion

Renaissance Revival is based on the silhouettes of the era. Common attire includes corsets, Elizabethan ruffle collars, bodices and long dresses.

## Media

### Movies

* Queen Margot (1994)
* Romeo + Juliet (1996)
* Dangerous Beauty (1998)
* Elizabeth (1998)
* Ever After (1998)
* Shakespeare in Love (1998)
* A Midsummer Night's Dream (1999)
* The Messenger: The Story of Joan of Arc (1999)
* Get Over It (2001)

## Gallery

R.E.M. - Losing My Religion (Official HD Music Video)-2

Windows XP Plus Theme - da Vinci

Enya - Book Of Days (Official Music Video)

## Sources

* <https://www.facebook.com/groups/819131621545040>
* <https://cari.institute/aesthetics/renaissance-revival>

## Images

- `images/renaissance-revival/Sixpence.jpg`
- `images/renaissance-revival/R-2195662-1462072374-9506.jpg`
- `images/renaissance-revival/R-2195662-1673711128-2582.jpg`
- `images/renaissance-revival/R-2195662-1673711130-1736.jpg`
- `images/renaissance-revival/R-2195662-1673711129-1955.jpg`
- `images/renaissance-revival/R-2195662-1673711130-2679.jpg`
- `images/renaissance-revival/Bjork.webp`
- `images/renaissance-revival/Bio-morph.png`
- `images/renaissance-revival/Madonna-as-norma-from-a-promo-poster-for-the-1999-mtv-video-v0-x04zygmv86nd1.webp`
- `images/renaissance-revival/Y2k-or-1990s-renaissance-revival-vibes-v0-h4nnh9rerkgf1.jpg`

## Source and attribution

Text adapted from "Renaissance Revival" on the Aesthetics Wiki (https://aesthetics.fandom.com/wiki/Renaissance_Revival), licensed CC-BY-SA 4.0. Retrieved 2026-07-22. Images retain their individual licenses; see the local `credits.json`.
