---
name: "Clockpunk"
slug: "clockpunk"
aka: ["Machinepunk"]
decade_of_origin: "Renaissance inspiration (14th–17th centuries)"
key_motifs: ["Clockwork mechanisms", "gears", "springs", "automatons", "ornithopters", "polished lenses", "intricate carvings"]
key_colours: ["Gold", "bronze", "brass", "wood tones", "silver"]
palette: ["#B1AC96", "#0E0807", "#8A6D4D", "#5F5E54", "#653C1D", "#403C33"]
related_aesthetics: ["Baroque", "Dungeonpunk", "Italian Renaissance", "Steampunk"]
related_media: ["Assassin's Creed II (video game)", "Dishonored 2 (video game)", "Hugo (film)"]
source_url: "https://aesthetics.fandom.com/wiki/Clockpunk"
license: "CC-BY-SA-4.0"
retrieved: "2026-07-22"
image_count: 12
---

# Clockpunk

_Also known as: Machinepunk_

Needs Work 🛠️

This page requires improvement. Please help us by [expanding it](https://aesthetics.fandom.com/wiki/Clockpunk?action=edit). If you are unsure how to help, check the article guide [Format and Content](https://aesthetics.fandom.com/wiki/Format_and_Content "Format and Content").  
**Reason:** This page's gallery is being spammed with Steampunk images. Clockpunk and Steampunk are distinct.

**Clockpunk** is a subgenre of speculative fiction similar to [Steampunk](https://aesthetics.fandom.com/wiki/Steampunk "Steampunk") that imagines a world where technology is based on Renaissance-era clockwork mechanics rather than steam power or electricity, being more surrealist than the latter. Drawing its aesthetic from the [Italian Renaissance](https://aesthetics.fandom.com/wiki/Italian_Renaissance "Italian Renaissance") and [Baroque](https://aesthetics.fandom.com/wiki/Baroque "Baroque") periods (roughly the 14th through 17th centuries), Clockpunk deals with the potential of intricate gears, springs, and levers to create advanced machinery.

## Visuals

The Clockpunk aesthetic is inspired by pre-industrial craftsmanship.

### Technology and Power

Clockpunk technology is purely mechanical, relying on the intricate interplay of gears, springs, and levers. Common inventions in Clockpunk settings include automatons (clockwork robots), ornithopters (flying machines with flapping wings), and other complex devices powered by winding mechanisms. Because the energy stored in a spring is limited, Clockpunk fiction often addresses the issue of power by incorporating fantastical elements that were part of the Renaissance worldview, such as alchemy or magic, to explain how its complex machines function.

### Visual Aesthetic and Materials

Visually, Clockpunk is highly ornate and detailed, mirroring the aesthetics of the Renaissance and [Baroque](https://aesthetics.fandom.com/wiki/Baroque "Baroque") eras. Machinery is not merely functional but is also treated as a work of art, featuring intricate carvings and elaborate embellishments. Unlike the industrial materials of Steampunk, Clockpunk favors materials like wood, brass, and bronze. The aesthetic also emphasizes precisely polished lenses and mirrors, reflecting the historical period's significant advancements in the field of optics.

## Distinction from [Steampunk](https://aesthetics.fandom.com/wiki/Steampunk "Steampunk")

| **Component** | **Steampunk** | **Clockpunk** |
| --- | --- | --- |
| Cultural Roots | Victoria-era London | Renaissance-era Italy |
| Visuals and Imagery | Industrial materials (iron, copper) |  |

## Media

### Literature

* Astro-Knights Island
* Mainspring by Jay Lake
* White Chapel Gods by S.M. Peters
* The Rithmatist by Brandon Sanderson
* Clockpunk Fairy Tales by Melanie Karsak
* The Mechanical by Ian Tregillis
* Twisted Cogs by Maddison Rose
* The Blazing World by Margaret Cavendish

### Film

* The Adventures of Baron Munchausen (1989)
* The Three Musketeers (2011)
* Alice In Wonderland
* Hugo (2011)
* Spy Kids 4: All the Time in the World (2011)

### Television

* Clockwork Planet (2017)
* Da Vinci's Demons (2013-2015)
* Leonardo (2011-2012)
* Hiwou War Chronicles (2000-2001)

### Video Games

* The Clockwork Mansion
* Dishonored 2
* Thief: The Dark Project
* Resonance of Fate
* Syberia
* Assassin Creed 2
* Okami (specifically the Hokkaido last part)
* Gorogoa
* The Clockwork City (Elder Scrolls Online DLC, Morrowind (Tribunal DLC))
* Clockwork Revolution
* Standstill Girl
* Fable II
* Bungo to Alchemist

## Gallery

## References

## Images

- `images/clockpunk/Clockpunk.jpg`
- `images/clockpunk/Clockpunk2.jpg`
- `images/clockpunk/Clockpunk10.png`
- `images/clockpunk/Connor-cercone-prof-layton-2018-final.jpg`
- `images/clockpunk/Clock-punk.jpg`
- `images/clockpunk/C9772c08506d4652f0ce63b761bad760.jpg`
- `images/clockpunk/Clockpunk-aesthetic.jpg`
- `images/clockpunk/Vo0yv9ybof5c1.webp`
- `images/clockpunk/Davinci-columbina.jpg`
- `images/clockpunk/Floff.jpg`
- `images/clockpunk/Remi-luizy-sr-cavalier-bs02.jpg`
- `images/clockpunk/Clockwork.webp`

## Source and attribution

Text adapted from "Clockpunk" on the Aesthetics Wiki (https://aesthetics.fandom.com/wiki/Clockpunk), licensed CC-BY-SA 4.0. Retrieved 2026-07-22. Images retain their individual licenses; see the local `credits.json`.
