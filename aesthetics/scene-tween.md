---
name: "Scene Tween"
slug: "scene-tween"
decade_of_origin: "2010s"
key_colours: ["Black", "green", "blue", "pink", "rainbow", "purple"]
related_aesthetics: ["Scene", "Kidcore", "McBling", "Tweencore", "Internet Awesomesauce", "Cyber Stylin'", "Groovival", "Nostalgiacore"]
source_url: "https://aesthetics.fandom.com/wiki/Scene_Tween"
license: "CC-BY-SA-4.0"
retrieved: "2026-07-22"
image_count: 16
---

# Scene Tween

Needs Work 🛠️

This page requires improvement. Please help us by [expanding it](https://aesthetics.fandom.com/wiki/Scene_Tween?action=edit). If you are unsure how to help, check the article guide [Format and Content](https://aesthetics.fandom.com/wiki/Format_and_Content "Format and Content").  
**Reason:** This page currently conflates two distinct aesthetics into one: 1) The [Rocker Grrl Diva](https://aesthetics.fandom.com/wiki/Rocker_Grrl_Diva "Rocker Grrl Diva") aesthetic (Late 90s/Early 00s Glam Rock Revival) and 2) The Scene Tween aesthetic (Late 00s/Early 10s commercialization of the [Scene](https://aesthetics.fandom.com/wiki/Scene "Scene") subculture). The page suffers from severe chronological confusion (infobox says 2010s; history describes 1990s origins).

**Scene Tween** is an aesthetic that emerged approximately in the early 2010s, combining elements of [Kidcore](https://aesthetics.fandom.com/wiki/Kidcore "Kidcore") and the [Scene](https://aesthetics.fandom.com/wiki/Scene "Scene") subculture. This fusion arose as mainstream culture began marketing the original [Scene](https://aesthetics.fandom.com/wiki/Scene "Scene") aesthetic towards children, much like [Teenpunk](https://aesthetics.fandom.com/wiki/Teenpunk "Teenpunk") with its [emo](https://aesthetics.fandom.com/wiki/Emo "Emo")-inspired imagery, but targeting an even younger audience. Scene Tween retains many of the visual elements of [Scene](https://aesthetics.fandom.com/wiki/Scene "Scene"), such as animal patterns and bright colours, but with a distinctly childish and feminine twist.

## History

In the late 2000s when the Scene emerged among teens, it began to be stripped of its original roots in order to appeal to young children of the time, with the style peaking in tween brands in the 2010s such as Justice.

It too got deemed as a "diva/sassy" type of aesthetic and was more youthful and feminine much like its precursor.

## Visuals

Scene Tween and Scene, as mentioned above, share many similar motifs and visuals, such as:

* Bright colors
* Animal print patterns
* Animals (especially in fashion)
* Rainbows
* Neon colors
* Glitter
* Peace signs
* Hearts
* Positive messages

## Media

### Franchises

* My Little Pony
* The Honey Girls
* Domo
* Care Bears
* Hello Kitty
* Monster High
* Littlest Pet Shop

### Television and Cartoons

Television from the Scene Tween era mainly consists of children's cartoons from said time period.

#### Television

* iCarly (2007-2012)
* Zoey 101 (2005-2008)
* Victorious (2010-2013)
* Make It Pop (2015-2016)

#### Cartoons

* My Little Pony: Friendship Is Magic (2010-2019)
* Littlest Pet Shop (2012-2016)
* Hanazuki: Full of Treasures (2016-2019)
* Strawberry Shortcake's Berry Bitty Adventures (2010-2015)
* Popples (2015-2016)
* Rainbow Butterfly Unicorn Kitty (2019)
* Luna Petunia (2016-2018)
* Powerpuff Girls (1998-2005)
* Rainbow Rangers (2018-2022)
* Unikitty! (2017-2020)

### Toys

* I Love VIP Pets
  + Original run (2013-2017)
  + Reboot (2020-present)
* Kittipillars/Pupillions
* LOL Surprise! Tweens: Emma Emo

### Video Games

* Neopets (1999-present)
* MovieStarPlanet (2009-present)
* Fantage (2008-2018)
* StarSue (2006-2020)
* Stardoll (2004-present)
* BooniePlanet (2014)

## Resources

### Brands

* Tokidoki
* So So Happy
* Lisa Frank
* Furby
* Faded Glory
* Sweet n' Sassy
* Hot Focus
* Webkinz
* Twinkle Toes by Sketchers
* Build-A-Bear

## Gallery

The Webkinz Rockerz Cat, which was released around the height of Scene Tween

2014 Furby Boom, from Hasbro

Lisa Frank

Make it Pop (2015-2016)

## Images

- `images/scene-tween/Scenetween.jpg`
- `images/scene-tween/Honeygirls.jpg`
- `images/scene-tween/Pillowhead.jpg`
- `images/scene-tween/Bestofallbestfriends.jpg`
- `images/scene-tween/Zoomeez.jpg`
- `images/scene-tween/Liviabedroom.jpg`
- `images/scene-tween/Nyan.jpeg`
- `images/scene-tween/Sockzz.jpeg`
- `images/scene-tween/LivvieItUp.jpeg`
- `images/scene-tween/Bffforever.jpeg`
- `images/scene-tween/SceneTween.jpeg`
- `images/scene-tween/Rocker.jpeg`
- `images/scene-tween/OldMLP.jpg`
- `images/scene-tween/FurbyBoomNostalgia.webp`
- `images/scene-tween/LisaFrankSceneTween.jpg`
- `images/scene-tween/06MAKEIT-superJumbo.jpg`

## Source and attribution

Text adapted from "Scene Tween" on the Aesthetics Wiki (https://aesthetics.fandom.com/wiki/Scene_Tween), licensed CC-BY-SA 4.0. Retrieved 2026-07-22. Images retain their individual licenses; see the local `credits.json`.
